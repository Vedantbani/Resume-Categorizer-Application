...
# # Load Job Role Details Dataset (excluding Resume column)
# job_details_df = pd.read_csv("IT_Job_Details_Dataset.csv")[['Job Role', 'Skills Required', 'Salary Estimate', 'Top Companies', 'Industry', 'Growth Path']]
...
#     details = job_details_df[job_details_df['Job Role'].str.lower() == job_role.lower()]
...
# def predict_jobs_top3(categorized_df):
#     final_results = []
#     for _, row in categorized_df.iterrows():
#         for idx, role in enumerate([row['Top 1 Role'], row['Top 2 Role'], row['Top 3 Role']], 1):
#             job_info = get_job_details(role)
#             if job_info:
#                 result = {
...
# st.set_page_config(page_title="📂 Resume Categorizer & IT Job Predictor", page_icon="📄", layout="wide")
...
# st.markdown("""
#     <h2 style='text-align: center;'>📄 Resume Categorizer & IT Job Predictor (Company Details)</h2>
#     <p style='text-align: center; color: #dddddd;'>Upload resumes, categorize into top 3 IT roles, and see applicable company and job details.</p>
#     <hr>
# """, unsafe_allow_html=True)
...
#     st.markdown("""
#     <h4 style='text-align: center;'>Predict IT Job & Company Details</h4>
#     """, unsafe_allow_html=True)

#     if st.button("🚀 Predict Job & Company Details"):
#         st.session_state['show_prediction'] = True
#         categorized_df = st.session_state['categorized_df']
#         predicted_df = predict_jobs_top3(categorized_df)
#         st.session_state['predicted_df'] = predicted_df
#         st.success("🎉 Job & company details prediction completed successfully!")

# if st.session_state.get('show_prediction', False):
#     predicted_df = st.session_state.get('predicted_df')
#     if predicted_df is not None:
#         st.dataframe(predicted_df)
#         results_csv = predicted_df.to_csv(index=False).encode('utf-8')
#         st.download_button(
#             label="⬇ Download job & company details as CSV",
#             data=results_csv,
#             file_name='resume_job_company_predictions.csv',
#             mime='text/csv'
#         )
...
# Streamlit Page Setup
st.set_page_config(page_title="📂 Stylish Resume Categorizer & Job Recommender", page_icon="📄", layout="wide")
